...
- 핵심 선택: 어떤 카드를 강화하고, 언제 개별 판매할 것인가

카드는 1장만 보유한 순간부터 언제든 강화하거나 판매할 수 있고, 5장을 모두 모은 뒤 각 카드를 차례로 강화해도 된다. 강화와 판매의 대상·순서·시점은 사용자가 자유롭게 정한다. 카드를 파괴하는 v1의 손실 구조를 제거하며, 강화에 실패한 카드는 현재 단계를 유지하되 추가 강화만 잠기므로 판매 가치는 보존된다.
...
- 카드 5장을 강화하지 않고 개별 판매할 때 최소 광고 10회
- 카드 5장을 성공 또는 실패까지 계속 강화하고 개별 판매하면 평균 광고 약 33.3회
- 목표 총 eCPM: 5,000원 이상
...
| 판매 | 선택한 카드 1장의 가치, 첫 판매 원소 보너스, 판매 광고 대기 상태 |
...
기존 v1 서버의 단일 카드 판매 흐름은 유지할 수 있지만, 광고 검증·v2 가치·첫 판매 보너스는 별도 마이그레이션이 필요하다. 파괴 강화, 3,000결정 환율과 교환 한도는 v2 규칙으로 교체하고 기존 데이터를 보존한다.

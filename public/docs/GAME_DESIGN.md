...
- 핵심 선택: 어떤 카드를 원하는 순서로 강화하고, 몇 장을 묶어 판매할 것인가

카드는 1장만 보유한 순간부터 언제든 강화하거나 판매할 수 있고, 판매할 때는 1~5장을 선택할 수 있다. 강화할 때마다 보유 카드 중 원하는 카드 한 장을 자유롭게 선택하며 획득 순서나 보관 위치에 따른 제한은 없다. 카드를 파괴하는 v1의 손실 구조를 제거하며, 강화에 실패한 카드는 현재 단계를 유지하되 추가 강화만 잠기므로 판매 가치는 보존된다.
...
- 매일 KST 00:00 변경
- 하루 첫 판매 1회만 적용
- 첫 판매로 선택한 카드 중 오늘의 원소 카드만 1.5배
- 이후 판매는 모두 1.0배
...
- 카드 5장을 강화하지 않고 한 번에 판매할 때 최소 광고 6회, 개별 판매 시 10회
- 카드 5장을 끝까지 강화한 뒤 한 번에 판매하면 평균 광고 약 29.3회, 개별 판매 시 약 33.3회
- 목표 총 eCPM: 5,000원 이상
...
| 판매 | 선택한 카드 1~5장의 가치 합계, 첫 판매 원소 보너스, 판매 광고 대기 상태 |
...
- 24시간 기후와 6일 랜덤 주머니
- 하루 첫 판매 보너스 사용 여부
- 카드 1~5장 선택 판매와 결정 원장
- 포인트 교환과 지급 상태
- 멱등성, 다계정·매크로·광고 조작 탐지

기존 v1 서버의 단일 카드 판매 흐름은 개별 판매에 활용할 수 있지만, 복수 카드 트랜잭션·광고 검증·v2 가치·첫 판매 보너스는 별도 마이그레이션이 필요하다. 파괴 강화, 3,000결정 환율과 교환 한도는 v2 규칙으로 교체하고 기존 데이터를 보존한다.

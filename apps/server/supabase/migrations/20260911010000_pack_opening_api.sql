-- Atomic V2 card-pack opening while keeping the documented three-table model.
ALTER TABLE public.users
  ADD COLUMN pack_opened_on date,
  ADD COLUMN pack_open_count integer NOT NULL DEFAULT 0
    CHECK (pack_open_count BETWEEN 0 AND 20),
  ADD COLUMN pack_request_receipts jsonb NOT NULL DEFAULT '{}'::jsonb
    CHECK (jsonb_typeof(pack_request_receipts) = 'object');
...
BEGIN
  ...
  END IF;
  IF v_user.pack_open_count >= 20 THEN RAISE EXCEPTION 'DAILY_PACK_LIMIT_REACHED'; END IF;
  ...
  SET pack_open_count = pack_open_count + 1,
      pack_request_receipts = pack_request_receipts || jsonb_build_object(p_request_id, v_result)
  WHERE id = p_user_id;
...
REVOKE ALL ON FUNCTION public.open_game_pack(bigint, text, bigint)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.open_game_pack(bigint, text, bigint) TO service_role;

...
async function reservePackOpening(request: Request): Promise<Response> {
  const userId = await requireSessionUser(request);
  const requestId = readRequestId(request);
  if (!requestId) return json({ code: 'INVALID_IDEMPOTENCY_KEY' }, 400);
...
async function openPack(request: Request): Promise<Response> {
  const userId = await requireSessionUser(request);
  const requestId = readRequestId(request);
  if (!requestId) return json({ code: 'INVALID_IDEMPOTENCY_KEY' }, 400);
...
function databaseError(message: string): Response {
  const code = [
    ...
    'PACK_AD_RESERVATION_REQUIRED',
    'PACK_AD_RESERVATION_EXPIRED',
